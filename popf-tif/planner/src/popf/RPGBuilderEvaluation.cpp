...
namespace Planner
{
    ...
    struct SupportingAction {
        ...
        SupportingAction(const int & actionID, const VAL::time_spec & timeSpecifier,
                         const int & applyThisManyTimes, const EpsilonResolutionTimestamp & tilRelevance,bool ift)
        : actID(actionID), ts(timeSpecifier),
        howManyTimes(applyThisManyTimes), tilR(tilRelevance),isFromTIFs(ift) {
        }
        SupportingAction(const int & actionID, const VAL::time_spec & timeSpecifier,
                         const int & applyThisManyTimes, const EpsilonResolutionTimestamp & tilRelevance)
        : actID(actionID), ts(timeSpecifier),
        howManyTimes(applyThisManyTimes), tilR(tilRelevance),isFromTIFs(false) {
        }
    ...
    struct ActionAndHowManyTimes {
        ...
        : actID(act), ts(timeSpec), howManyTimes(lim), minDur(dmin), maxDur(dmax), gradientsFinishAt(EpsilonResolutionTimestamp::infinite()), gradientNeedsStarting(true), alreadyInThePlan(alreadyIn)
        {
            ...
            assert(ts != VAL::E_AT || howManyTimes == 1);
        }
    ...
    struct FactLayerEntry {
        ...
        /** @brief Timed Initial Literal Events */
        list<int> TILs;
    ...
    public:
        ...
        protected:
            ...
            void changeInternalValue(map<int,double> toChange){
                if(internalValues.size()> 18446726){
                    return;
                }
                map<int,double>::iterator toChIt = toChange.begin();
                for(;toChIt!=toChange.end();++toChIt){
                    int id = toChIt->first;
                    double val = toChIt->second;
                    //cout << id << " " << val << " " << internalValues.size() << endl;
                    if(id<internalValues.size())
        ...
        public:
            ...
            const set<int> * getInstantaneousEffectsThatIncreasedVariable(const int & v) const {
                ...
                if (v > effectsThatIncreasedThisVariable.size()){
                    return 0;
                }
    ...
    public:
        ...
        void applyTIFRecordedEffects(EpsilonResolutionTimestamp achievedInLayer, map<int, double> & newEffect) {
            
            LayerMap::iterator layIt = layers.begin();
            for(;layIt!=layers.end();++layIt){
                EpsilonResolutionTimestamp time = layIt->first;
                if(time == achievedInLayer){
                    break;
                }
            }
            
            if (debug) {
                cout << "Applying the TIF numeric effects noted in this layer; effects affect " << achievedInLayer << endl;
            }
            
            set<int> variableChanged;
            
            map<int, double>::iterator newEffIt = newEffect.begin();
            
            for(;newEffIt!=newEffect.end();++newEffIt){
                int affectedVar = newEffIt->first;
                double maxVal = newEffIt->second;
                variableChanged.insert(affectedVar);
            }
            layIt->second->changeInternalValue(newEffect);
            
            newEffIt = newEffect.begin();
            for(;newEffIt!=newEffect.end();++newEffIt){
                int affectedVar = newEffIt->first;
                double maxVal = newEffIt->second;
                variableChanged.insert(affectedVar);
        ...
        void satisfyNumericPreconditionsAtLayer(RPGRegressionMap::const_iterator & currentLayer,
                                                RPGRegressionMap & goalsAtLayer,
                                                list<pair<EpsilonResolutionTimestamp, SupportingAction> > & actionsUsed, set<int> & actionsAlreadyUsed, map<pair<int,VAL::time_spec>,double> & anticipating, map<int, set<int> > startedActions, bool isTIF=false) {
            
            if (currentLayer->second.numericValueGreaterThan.empty()) return;
            if (currentLayer->first <= EpsilonResolutionTimestamp::zero()) return;
            EpsilonResolutionTimestamp timeNow = currentLayer->first;
            //bool debug = true;
            if (debug) {
                cout << COLOUR_light_blue << "Satisfying numeric preconditions in layer " << currentLayer->first ;
                if(isTIF){
                    cout << " for tif ";
                }
                cout << COLOUR_default  << endl;
            }
            map<pair<int,VAL::time_spec>,double>  bookeeping;
            map<pair<int,VAL::time_spec>,bool> actionFirst;
            LayerMap::iterator layerBeforeNow = layers.begin();
            if(isTIF){
                for(;layerBeforeNow!=layers.end();++layerBeforeNow){
                    if (layerBeforeNow->first==timeNow){
                        break;
                    }
                }
            }
            LayerMap::iterator layersecond = layerBeforeNow;//reachableValues;
            layersecond--;
            
            const LayerMap::iterator preconditionsIn = isTIF?layersecond:layers.find(currentLayer->first);
            ...
            for (; thresholdItr != thresholdEnd; ++thresholdItr) {
                ...
                if (!isTIF){
                    ...
                    previousValue=borrowFactLayerZeroValues()[thresholdItr->first];
                    
                    if (needToAchieve < previousValue) {
                        // have already satisfied this when satisfying something else
                        if (debug) {
                            cout << "- Already have enough " << thresholdItr->first << ", since it is now "  << needToAchieve << " < " << previousValue << endl;
                        }
                        continue;
                    }
                    if (debug) {
                        cout << "- Not have enough " << thresholdItr->first << ", since it is now "  << needToAchieve << " >= " << previousValue << endl;
                    }
                ...
                
                {
                    ...
                    if (assignmentUsed && assignmentUsed->act) {
                        ...
                        actionsUsed.push_back(make_pair(currentLayer->first,SupportingAction(toUse->actID, toUse->ts, 1, thresholdItr->second.second,isTIF)));
                        
                        if (debug) {
                            if (toUse->ts == VAL::E_AT) {
                                cout << "  * Using assignment by TIL at time " << RPGBuilder::getTILVec()[toUse->actID]->duration << endl;
                            } else {
                        ...
                        if (currEff.size) {
                            ...
                            breakApart(currEff.variables, currEff.weights, currEff.size, currEff.constant,
                                       ...
                                       isTIF?layersecond:previousFluentLayer, RPGBuilder::getOpMinDuration(toUse->actID,0), RPGBuilder::getOpMaxDuration(toUse->actID,0), subTerms,timeNow, isTIF);
                ...
                if (!assignmentMatched) {
                    ...
                    if (thresholdItr->first < 2 * FluentLayerEntry::pneCount) {
                        ...
                        breakApart(currAV.fluents, currAV.weights, currAV.size, currAV.constant,
                                   true, avRemaining,
                                   isTIF?layersecond:preconditionsIn, EPSILON, DBL_MAX, subTerms,timeNow, isTIF);
                    }
                    ...
                    for (; tItr != tEnd; ++tItr) {
                        ...
                        const double acceptableMinimum = isTIF?layerBeforeNow->second->values()[tItr->first]:previousFluentLayer->second->values()[tItr->first];
                        
                        if (debug) {
                            cout << "- Considering ways of ";
                            stringstream number;
                            if(acceptableMinimum==DBL_MAX)
                                number << "inf ";
                            else
                                number << acceptableMinimum;
                            if (tItr->first < FluentLayerEntry::pneCount) {
                                cout << "increasing " << *(RPGBuilder::getPNE(tItr->first)) << ": must have " << residual << " by now, given " << number.str() << " previously\n";
                            } else {
                        ...
                        if (!localAssignmentMatched) {
                            ...
                            set<int> instantaneousEffs;
                            map<int, double> effectTime;
                            
                            map<pair<int,int>,double> actionEffectTime;
                            map<pair<int,int>, EpsilonResolutionTimestamp> actionEffectLayer;
                            const set<int> *test = (isTIF?layersecond:preconditionsIn)->second->getInstantaneousEffectsThatIncreasedVariable(tItr->first);
                            if (test){
                                instantaneousEffs = *test;
                            }
                            if(isTIF){
                                while(layersecond!=layers.begin()){
                                    
                                    const set<int> * const  instantaneousEffs2 = (isTIF?layersecond:preconditionsIn)->second->getInstantaneousEffectsThatIncreasedVariable(tItr->first);
                                    if (instantaneousEffs2 && !instantaneousEffs2->empty()){
                                        //instantaneousEffs.clear();
                                        instantaneousEffs.insert(instantaneousEffs2->begin(),instantaneousEffs2->end());
                                        for(set<int>::iterator sItr = instantaneousEffs2->begin();sItr!=instantaneousEffs2->end();++sItr){
                                            effectTime.insert(make_pair(*sItr,layersecond->first.toDouble()));
                                        }
                                    }
                                    if (debug) {
                                        if (instantaneousEffs2 && !instantaneousEffs2->empty()) {
                                            cout << "  * Considering instantaneous effects " << layersecond->first << "\n";
                                        } else {
                            ...
                            //const set<int> * const instantaneousEffs = (isTIF?layersecond:preconditionsIn)->second->getInstantaneousEffectsThatIncreasedVariable(tItr->first);
                            if (!instantaneousEffs.empty()) {
                                set<int>::const_iterator effItr = instantaneousEffs.begin();
                                const set<int>::const_iterator effEnd = instantaneousEffs.end();
                                // ipsa
                                int nTimes =  DBL_MAX;
                                double nTimeFloating = nTimes;
                                double nTimeInt = nTimes;
                                int minEffTime=-1;
                                pair<int,int> minActionTime(-1,-1);
                                bool isFirst = true;
                                ...
                                for (; effItr != effEnd /*&& residual > acceptableMinimum*/; ++effItr) {
                                    ...
                                    if (debug) {
                                        cout << "      - One potential effect: " << currEff << " " << *effItr << endl;
                                    }
                                    layersecond = layerBeforeNow;//reachableValues;
                                    layersecond--;
                                    
                                    list<const ActionAndHowManyTimes*>  actions = (isTIF?layersecond:preconditionsIn)->second->getInstantaneousEffectsDirectlyBeforeThisLayer()[*effItr];
                                    if(isTIF){
                                        if(!actions.empty()){
                                            for(list<const ActionAndHowManyTimes*>::iterator a2 = actions.begin(); a2!=actions.end();++a2){
                                                actionEffectTime[make_pair((*a2)->actID,*effItr)] =layersecond->first.toDouble();
                                                //						 cout << "** update min action time " << *(RPGBuilder::getInstantiatedOp((*a2)->actID)) << " " << (*a2)->howManyTimes << " " << layersecond->first.toDouble() << endl;
                                                if(isFirstAction && isFirst){
                                                    minActionTime = make_pair((*a2)->actID,*effItr);
                                                    //cout << "** update min action time" << (*a2)->actID << " " << *effItr << endl;
                                                    isFirstAction = false;
                                                }
                                            }
                                        }
                                        while(layersecond!=layers.begin()){
                                            list<const ActionAndHowManyTimes*>  actions2 = layersecond->second->getInstantaneousEffectsDirectlyBeforeThisLayer()[*effItr];
                                            if(!actions2.empty()){
                                                for(list<const ActionAndHowManyTimes*>::iterator a2 = actions2.begin(); a2!=actions2.end();++a2){
                                                    if(actionEffectTime.find(make_pair((*a2)->actID,*effItr))!=actionEffectTime.end()){
                                                        //							 continue;
                                                    }
                                                    actionEffectTime[make_pair((*a2)->actID,*effItr)] =layersecond->first.toDouble();
                                                    //                                                     cout << "** update min action time " << *(RPGBuilder::getInstantiatedOp((*a2)->actID)) << " " << (*a2)->howManyTimes << " " << layersecond->first.toDouble() << endl;
                                    ...
                                    for (; aItr != aEnd /*&& residual > acceptableMinimum*/; ++aItr) {
                                        ...
                                        
                                        {
                                            ...
                                            if (tItr->first >= FluentLayerEntry::pneCount ? (minEff < 0 || maxEff < 0) : (minEff > 0 || maxEff > 0)) {
                                                ...
                                                double applyTimesFloating = applyTimes;
                                                bookeeping[make_pair((*aItr)->actID,(*aItr)->ts)] =  actionEffectTime[make_pair((*aItr)->actID,*effItr)];
                                                if (debug) {
                                                    cout << "          + Effect can be obtained " << applyTimes << "/" << INT_MAX << " time(s) using ";
                                                    if ((*aItr)->ts == VAL::E_AT_START) {
                                                        cout << "start of ";
                                                    } else {
                                                        cout << "end of ";
                                                    }
                                                    cout << *(RPGBuilder::getInstantiatedOp((*aItr)->actID)) << ", yielding an effect value of " << localEff << " ( " << currEff.ID << " " << currEff << " )" <<endl;
                                                }
                                                map<int,set<int> >::iterator sit = startedActions.begin();
                                                
                                                if (actionsAlreadyUsed.find((*aItr)->actID)!=actionsAlreadyUsed.end()/* || startedActions.find((*aItr)->actID) != startedActions.end()*/ ){
                                                    if (debug) {
                                                        cout << "action already used" << endl;
                                                    }
                                                    actionsUsed.clear();
                                                    // change this
                                                    if (isTIF)
                                                        anticipating[make_pair((*aItr)->actID,(*aItr)->ts)]= actionEffectTime[make_pair((*aItr)->actID,*effItr)];
                                                    return;
                                                }
                                                ...
                                                map<int,set<int> >::iterator actionsStarted = startedActions.find((*aItr)->actID);
                                                //						 cout << "Going to apply it " << applyTimes << "(" << applyTimesFloating<< ") times, leaving a residual of " << residual << endl;
                                                
                                                if ((*aItr)->ts == VAL::E_AT_END && actionsStarted != startedActions.end() && applyTimes<=actionsStarted->second.size()){
                                                    if (debug) {
                                                        cout << "action already used as start" << endl;
                                                    }
                                                    if(!actionFirst[make_pair((*aItr)->actID,(*aItr)->ts)]){
                                                        actionsUsed.clear();
                                                        if(isTIF)
                                                            anticipating[make_pair((*aItr)->actID,(*aItr)->ts)]= actionEffectTime[make_pair((*aItr)->actID,*effItr)];
                                                        return;
                                                    ...
                                                    if ((*aItr)->ts == VAL::E_AT_END && actionsStarted != startedActions.end() && applyTimes>actionsStarted->second.size()){
                                                        applyTimes-=actionsStarted->second.size();
                                                        actionFirst[make_pair((*aItr)->actID,(*aItr)->ts)]=true;
                                                    }
                                                }
                                                if (debug) {
                                                    cout << "Going to apply it " << applyTimes << "(" << applyTimesFloating<< ") times, leaving a residual of " << residual << endl;
                                                ...
                                                bool test = false;
                                                if (applyTimes < nTimeInt){
                                                    //						     cout << " case 1 " << endl;
                                                    nTimeInt = applyTimes;
                                                    nTimeFloating = applyTimesFloating;
                                                    minEffTime = *effItr;
                                                    minActionTime=make_pair((*aItr)->actID,*effItr);
                                                    //cout << "update min action time" << (*aItr)->actID << " " << *effItr << endl;
                                                    actionsUsed.clear();
                                                    actionsUsed.push_back(make_pair(currentLayer->first,SupportingAction((*aItr)->actID, (*aItr)->ts, applyTimes, thresholdItr->second.second,isTIF)));
                                                }else if (applyTimes==nTimeInt){
                                                    
                                                    //						     cout << " case 2 " << *effItr << " " << minEffTime << " " << effectTime[*effItr] << " " << effectTime[minEffTime] << endl;
                                                    if(actionEffectTime[make_pair((*aItr)->actID,*effItr)]<actionEffectTime[minActionTime]){
                                                        //							 cout << " case 3" << endl;
                                                        nTimeInt = applyTimes;
                                                        nTimeFloating = applyTimesFloating;
                                                        minEffTime = *effItr;
                                                        ...
                                                        actionsUsed.push_back(make_pair(currentLayer->first,SupportingAction((*aItr)->actID, (*aItr)->ts, applyTimes, thresholdItr->second.second,isTIF)));
                                                    }else if (actionEffectTime[make_pair((*aItr)->actID,*effItr)]==actionEffectTime[minActionTime]){
                                                        //							 cout << "case 4" << endl;
                                                        if (applyTimesFloating < nTimeFloating){
                                                            //							     cout << "case 5" << endl;
                                                            nTimeInt = applyTimes;
                                                            nTimeFloating = applyTimesFloating;
                                                            minEffTime = *effItr;
                                    ...
                                    if (maxUsed) {
                                        ...
                                        breakApart(currEff.variables, currEff.weights, currEff.size, currEff.constant,
                                                   ...
                                                   isTIF?layersecond:previousFluentLayer, 0.001, DBL_MAX, inputSubTerms,timeNow, isTIF);
                                    ...
                                    for (; effItr != effEnd && residual > acceptableMinimum; ++effItr) {
                                        ...
                                        for (; aItr != aEnd && residual > acceptableMinimum; ++aItr) {
                                            ...
                                            if (nInt > (*aItr)->howManyTimes) {
                                                ...
                                                actionsUsed.push_back(make_pair(currentLayer->first,SupportingAction((*aItr)->actID, (*aItr)->ts, (*aItr)->howManyTimes, thresholdItr->second.second,isTIF)));
                                                recordSideEffects(*aItr, (*aItr)->howManyTimes, previousFluentLayer->second->values());
                                            } else {
                                                residual -= nInt * eff;
                                                actionsUsed.push_back(make_pair(currentLayer->first,SupportingAction((*aItr)->actID, (*aItr)->ts, nInt, thresholdItr->second.second,isTIF)));
                                ...
                                for (; effItr != effEnd && residual > acceptableMinimum; ++effItr) {
                                    ...
                                    for (; aItr != aEnd && residual > acceptableMinimum; ++aItr) {
                                        ...
                                        if (!(*aItr)->alreadyInThePlan) {
                                            actionsUsed.push_back(make_pair(layerWithGradientOnThisVar->first,SupportingAction((*aItr)->actID, (*aItr)->ts, needed, thresholdItr->second.second,isTIF)));
            ...
            for(list<pair<EpsilonResolutionTimestamp, SupportingAction> >::iterator aItr = actionsUsed.begin();aItr!=actionsUsed.end();++aItr){
                //cout << aItr->first << " " << *(RPGBuilder::getInstantiatedOp(aItr->second.actID)) << endl;
                actionsAlreadyUsed.insert(aItr->second.actID);
                if (isTIF)
                    anticipating[make_pair(aItr->second.actID,aItr->second.ts)] = bookeeping[make_pair(aItr->second.actID,aItr->second.ts)];
            }
            if (debug) {
        ...
        void breakApart(const vector<int> & variables, const vector<double> & weights, const int & size, const double & constant,
                        ...
                        list<pair<int,double> > & subTerms, EpsilonResolutionTimestamp timeNow, bool isTIF) {
            ...
            const vector<double> & initValues = layerBeforeNow->second->values();
            //const vector<double> & values = isTIF?layersecond->second->values():reachableValues->second->values();
            ...
            for (int s = 0; s < size; ++s) {
                ...
                if (v == -5) {
                    if (current_analysis->the_problem->metric->opt == VAL::E_MINIMIZE) {
                        accumulatedLHS -= w * Globals::bestSolutionQuality;
                    } else {
                        accumulatedLHS += w * Globals::bestSolutionQuality;
                    }
                    continue;
            ...
            for (int s = 0; s < size && accumulatedLHS < threshold; ++s) {
                ...
                // cout << "s : " << s << " v : " << v << " values[v] " << values[v] << " initialValues[v] " << initValues[v] << endl;
                
                if (initValues[v] == values[v]) {
                    if (debug) {
                        //cout << *(RPGBuilder::getPNE(v)) << endl;
                ...
                if (isTIF && values[v]<DBL_MAX) {
                    if (debug) {
                        cout << "Skipping over TIF\n";
...
#define MAKESTARTMDI(dest,a) MaxDependentInfo dest = a
        ...
        void addTemporalConstraintsFromActiveActions(BuildingPayload * const payload) {
            ...
            for (; constrItr != constrEnd; ++constrItr) {
                ...
                for (int pass = 0; pass < 2; ++pass) {
                    ...
                    for (; effItr != effEnd; ++effItr) {
                        if (effItr->first<RPGBuilder::rogueActions.size() &&  !RPGBuilder::rogueActions[effItr->first]) {
        ...
        void lookAhead(BuildingPayload * const payload, int nLookAhead, RPGRegressionMap &goalsAtLayer, map<EpsilonResolutionTimestamp, bool>& isPreconditionFromTIF, bool debug=true){
            //          debug = true;
            ...
            int nextTIL = theState.nextTIL;
            if(debug){
                cout << "nextTil " << nextTIL << " " << nLookAhead << endl;
            }
            int countTIL = 0;
            for(int iTIL = 0; iTIL<nLookAhead; countTIL++){
                int tilID = nextTIL + countTIL;
                if(tilID >= RPGBuilder::getTILVec().size()){
                    break;
                }
                if (RPGBuilder::getTilNumerics()[tilID].empty())
                    continue;
                iTIL++;
                double tilTime = RPGBuilder::getTILVec()[tilID]->duration;
                if(debug)
                    cout << "visiting " << tilTime << endl;
                
                // apply effect of TILs
                MinimalState nextState = theState;
                list<int>::iterator effItr = RPGBuilder::getTilNumerics()[tilID].begin();
                list<int>::iterator effItrEnd = RPGBuilder::getTilNumerics()[tilID].end();
                
                for (;effItr!=effItrEnd;++effItr){
                    RPGBuilder::RPGNumericEffect & currEff = RPGBuilder::getNumericEff()[*effItr];
                    currEff.applyEffectMinMax(theState,nextState);
                }
                
                if(ExternalSolver::isActive){
                    MinimalState nextStateWithExSo = nextState;
                    ExternalSolver::count = 0;
                    list<int>::iterator effItr = RPGBuilder::getFakeExSoEffNumerics().begin();
                    list<int>::iterator effItrEnd = RPGBuilder::getFakeExSoEffNumerics().end();
                    for (;effItr!=effItrEnd;++effItr){
                        RPGBuilder::RPGNumericEffect & currEff = RPGBuilder::getNumericEff()[*effItr];
                        currEff.applyEffectMinMaxWithExternalEvaluation(nextState,nextStateWithExSo);
                    }
                    nextState=nextStateWithExSo;
                    //
                }
                
                // check if condition there are some conditions not satisfied
                set<int> precViolated;
                map<int, set<int> >::const_iterator itr = theState.startedActions.begin();
                const map<int, set<int> >::const_iterator itrEnd = theState.startedActions.end();
                for (; itr != itrEnd; ++itr) {
                    if(openEndActionSchedule[itr->first].toDouble()<tilTime){
                        continue;
                    }
                    ...
                    list<int>::iterator   itrInv = RPGBuilder::getInvariantNumerics()[itr->first].begin();
                    list<int>::iterator itrInvEnd = RPGBuilder::getInvariantNumerics()[itr->first].end();
                    for(; itrInv != itrInvEnd; ++itrInv){
                        const RPGBuilder::RPGNumericPrecondition currPre = RPGBuilder::getNumericPreTable()[*itrInv];
                        if (!currPre.isSatisfiedWCalculate(nextState.secondMin, nextState.secondMax)){
                            //alreadyAViolation=  true;
                            precViolated.insert(*itrInv);
                            if (debug) cout << " checking " << currPre;
                            if (debug) cout << " not satisfied." << endl;
                ...
                map<int,double> recordedEffects;
                vector<double> newValues = nextState.secondMin;
                
                {
                    for (int i=0;i<RPGBuilder::getPNECount();++i){
                        recordedEffects[i]=nextState.secondMin[i];
                    }
                    for (int i=RPGBuilder::getPNECount(); i<2*RPGBuilder::getPNECount();i++) {
                        newValues.push_back(-nextState.secondMin[i-RPGBuilder::getPNECount()]);
                        recordedEffects[i]=-nextState.secondMin[i-RPGBuilder::getPNECount()];
                    }
                    for (int i=2*RPGBuilder::getPNECount(); i<2*RPGBuilder::getPNECount()+RPGBuilder::getAVCount();i++) {
                        const vector<double> toEvaluating = newValues;
                        RPGBuilder::ArtificialVariable newAV = RPGBuilder::getArtificialVariable(i);
                        recordedEffects[i]=newAV.evaluate(toEvaluating);
                        newAV.evaluate(toEvaluating);
                    }
                }
                payload->fluentLayers.applyTIFRecordedEffects(EpsilonResolutionTimestamp(tilTime,true), recordedEffects);
                
                // add goal
                set<int>::iterator precVIt = precViolated.begin();
                set<int>::iterator precVEnd = precViolated.end();
                for(;precVIt != precVEnd; ++precVIt){
                    const int currGoal = *precVIt;
                    EpsilonResolutionTimestamp tilLayer(tilTime, true);
                    const EpsilonResolutionTimestamp & insLayer = tilLayer;
                    payload->fluentLayers.requestGoal(currGoal, insLayer, goalsAtLayer);
                    isPreconditionFromTIF.insert(make_pair(insLayer,true));
                }
        ...
        bool extractRP(BuildingPayload * const payload, int & h, list<pair<double, list<ActionSegment> > > & relaxedPlan, pair<int, VAL::time_spec> & earliestTIL, double & makespanEstimate, map<pair<int,VAL::time_spec>,double> & anticipating) {
            
            makespanEstimate = 0.0;
            map<EpsilonResolutionTimestamp, bool> isPreconditionFromTIF;
            ...
            set<int> actionsAlreadyUsed;
            {
                ...
                if (!ignoreNumbers) {
                    ...
                    if(LookAhead::isActive){
                        lookAhead(payload, LookAhead::nLook, goalsAtLayer, isPreconditionFromTIF,false);
                    }
                }
                ...
                while (!goalsAtLayer.empty()) {
                    ...
                    if (!ignoreNumbers) {
                        ...
                        map<int, set<int> > startedActions = payload->startState.startedActions;
                        payload->fluentLayers.satisfyNumericPreconditionsAtLayer(thisLayer, goalsAtLayer, actionsUsed,actionsAlreadyUsed,anticipating,startedActions);
                        
                        bool thisPrecIsGoal = false;
                        if(isPreconditionFromTIF.find(thisLayer->first)!=isPreconditionFromTIF.end())
                            thisPrecIsGoal = true;
                        if(thisPrecIsGoal)
                            payload->fluentLayers.satisfyNumericPreconditionsAtLayer(thisLayer, goalsAtLayer, actionsUsed,actionsAlreadyUsed, anticipating,startedActions,thisPrecIsGoal);
                        
                        if (!actionsUsed.empty()) {
                            if(ExternalSolver::heuristicActive && !ExternalSolver::timeToCalculate){
                                return false;
                            }
                            if (!alreadyPushed) {
                                relaxedPlan.push_front(pair<double, list<ActionSegment> >((currTS - EpsilonResolutionTimestamp::epsilon()).toDouble(), list<ActionSegment>()));
                            ...
                            for (; sapItr != sapEnd; ++sapItr) {
                                ...
                                if (currAchievedBy.second == VAL::E_AT_START) {
                                    ...
                                    assert(currAchievedBy.second == VAL::E_AT);
                                    
                                    if (evaluateDebug) cout << "\t\tUsing TIL at " << RPGBuilder::getTILVec()[currAchievedBy.first]->duration << endl;
                                    
                                    earliestTIL = currAchievedBy;
                                    
                                    {
                                        list<Literal*> & actionEffectsList = RPGBuilder::getTILVec()[currAchievedBy.first]->addEffects;
                                        list<Literal*>::iterator aelItr = actionEffectsList.begin();
                                        const list<Literal*>::iterator aelEnd = actionEffectsList.end();
            ...
            /** @brief recalculate Effects of action that invoke external solver
             *
             * @param theState state in which we apply the actionEffectsList
             * @param currAct action to apply
             * @param newEffects list of new effects (result of the calculation)
             */
            void recalculateRPGEffects(MinimalState theState, int currAct, const VAL::time_spec currTS, list<int> &newEffects);
        ...
        void RPGHeuristic::doFullExpansion(MinimalState & refState)
        {
            ...
            list<FFEvent>  partialPlan;
            map<double, list<pair<int, int> > > * justApplied = 0;
            double tilFrom = 0.001;
            getRelaxedPlan(refState, 0, minTimestamps, 0.0, refState.secondMin, refState.secondMax, timeAtWhichValueIsDefined, dummyHelpful, dummyRP, dummyEstimate,justApplied,tilFrom,partialPlan,false);
        ...
        int RPGHeuristic::getRelaxedPlan(const MinimalState & theState, const list<StartEvent> * startEventQueue,
                                         ...
                                         map<double, list<pair<int, int> > > * justApplied, double tilFrom, list<FFEvent>  partialPlan, bool timewindowsCheck, bool isRecalculated)
        {
            ...
            if(ExternalSolver::heuristicActive){
                ExternalSolver::timeToCalculate=isRecalculated;
                if(evaluateDebug && ExternalSolver::timeToCalculate) cout << "recalculating relaxed plan" << endl;
            }
            
            if (!d->expandFully) {
                if((TimewindowsChecker::relaxedCheck && timewindowsCheck) || isRecalculated || (!LookAhead::isActive && LookAhead::onlyH)  || (!LookAhead::isActive && LookAhead::onlyHA))
                    --statesEvaluated;
                ++statesEvaluated;
            }
            
            list<pair<int, FFEvent> > openTimewindows;
            list<FFEvent>  plan = partialPlan;
            if(!partialPlan.empty() && timewindowsCheck)
                openTimewindows = TimewindowsChecker::openOneShotTimewindowActions(partialPlan,theState);
            ...
            while (!nextHappening.empty()) {
                        ...
                        if (cTime > Private::latestEndAllowed[actToPass]) {
                            ...
                            if(timewindowsCheck){
                                ...
                                for(list<pair<int, FFEvent> >::iterator otIt = openTimewindows.begin();otIt!=openTimewindows.end(); ++otIt){
                                    //check if the action is affected by a timewindows action
                                    int idTWA = otIt->second.action->getID();
                                    if(RPGBuilder::isDependentOnTimewindow(actToPass,idTWA)){
                                        // find path in the relax plan
                                        TemporalConstraints cons = TemporalConstraints(*theState.temporalConstraints,2);
                                        list<StartEvent> open = (*startEventQueue);
                                        //                                 cout << "action " << *(RPGBuilder::getInstantiatedOp(actToPass)) << " ends at time " << nextHappening.endActionsAppearingAtThisTime->first.toDouble() << " is influenced by " << *(RPGBuilder::getInstantiatedOp(idTWA)) << endl;
                                        open.push_back(StartEvent(actToPass, 0,plan.size() - 2, nextHappening.endActionsAppearingAtThisTime->first.toDouble(), nextHappening.endActionsAppearingAtThisTime->first.toDouble(), 0.0));
                                        list<FFEvent> newEvents;
                                        
                                        ActionSegment actID(RPGBuilder::getInstantiatedOp(actToPass),VAL::E_AT_END,-1,RPGHeuristic::emptyIntList);
                                        TimewindowsChecker::addTemporalConstraint(cons,plan,otIt->first,actToPass);
                                        newEvents = TimewindowsChecker::addActionToHeader(plan,open,actID);
                                        
                                        ParentData * parent = LPScheduler::prime(plan, &cons, open);
                                        ChildData * child = parent->spawnChildData(open, plan, newEvents, false, &cons, plan.size()+1);
                                        if (!LPScheduler::checkTimewindow(child)){
                                            timeWindowValue = false;
                                            if(evaluateDebug){
                                                cout << "constraint not respected " << endl;
                                            }
                                            //                                       cout << "\n\t" << *(RPGBuilder::getInstantiatedOp(actToPass)) << " at time layer " << nextHappening.endActionsAppearingAtThisTime->first;
                                            //list<FFEvent> test = plan;
                                            //for(list<FFEvent>::iterator tIt = newEvents.begin(); tIt!=newEvents.end();++tIt){
                                            //    test.push_back(*tIt);
                                            //}
                                            //FF::printPlanAsDot(cout,test,&cons);
                                            delete parent;
                                            break;
                                        }else{
                                            if(evaluateDebug)
                ...
                if (nextHappening.newFactsAtThisTime != payload->factLayers.end()) {
                    ...
                    
                    {
                        ...
                        for (; stateItr != stateEnd; ++stateItr) {
                            if (evaluateDebug) {
                                cout << "Updating from TIL " << *stateItr << " @ " << RPGBuilder::getTILVec()[*stateItr]->duration << " (cTime = " << cTime << ")" << endl;
                            }
                            ...
                            if (d->applyPropositionalEffects(payload.get(), *stateItr, VAL::E_AT, false, nlTime) && breakOnGoalsAndEnds) {
                                break;
                            }
                            
                            if (d->applyNumericEffects(payload.get(), *stateItr, VAL::E_AT, nlTime, 1) && breakOnGoalsAndEnds) {
                                break;
                            }
                        }
                    }
                    ...
                    if (failedToMeetDeadline) {
                        if (evaluateDebug) {
                            cout << "Failed to meet a deadline\n";
                        }
                        break;
                    }
#endif
                ...
                if (payload->unsatisfiedGoals || payload->unappearedEnds
                    ...
                    pair<int, VAL::time_spec> earliestTIL(INT_MAX, VAL::E_AT);
                    map<pair<int,VAL::time_spec>,double> anticipating;
                    //d->extractRP(payload.get(), h, relaxedPlan, earliestTIL, finalPlanMakespanEstimate);
                    bool failed = d->extractRP(payload.get(), h, relaxedPlan, earliestTIL, finalPlanMakespanEstimate,anticipating);
                    if(ExternalSolver::heuristicActive && !failed){
                        //cout << "failed " << endl;
                        return getRelaxedPlan(theState,startEventQueue,minTimestamps,stateTS,
                                              extrapolatedMin,extrapolatedMax,timeAtWhichValueIsDefined,
                                              helpfulActions,relaxedPlan,finalPlanMakespanEstimate,
                                              justApplied,tilFrom,partialPlan,timewindowsCheck,true);
                    }
                    
                    
                    //rearrange the relax plan... with anticipated action first
                    if (LookAhead::isActive && !anticipating.empty() && ExternalSolver::isActive && false){
                        list<pair<double, list<ActionSegment> > > newRelaxedPlan;
                        list<pair<double, list<ActionSegment> > > anticipatedPlan;
                        map<double,bool> orderingTime;
                        map<pair<int,VAL::time_spec>,double>::iterator anItr = anticipating.begin();
                        for(;anItr!=anticipating.end();++anItr){
                            double time = anItr->second;
                            orderingTime[time] = true;
                            list<pair<double, list<ActionSegment> > >::iterator rItr = relaxedPlan.begin();
                            const list<pair<double, list<ActionSegment> > >::iterator rEnd = relaxedPlan.end();
                            list<ActionSegment> updatedActions;
                            for (;rItr!=rEnd;++rItr){
                                //double newTime = rItr->first;
                                {
                                    // check if there is a time in anticipating that is not is before
                                    double oldTime = rItr->first;
                                    list<ActionSegment> actions = rItr->second;
                                    list<ActionSegment>::iterator aItr = actions.begin();
                                    list<ActionSegment>::iterator aEnd = actions.end();
                                    for(;aItr!=aEnd;++aItr){
                                        int idAction = aItr->first->getID();
                                        VAL::time_spec ts = aItr->second;
                                        if(anticipating.find(make_pair(idAction,ts))!=anticipating.end() && time < oldTime){
                                            updatedActions.push_back(*aItr);
                                        }
                                    }
                                }
                                if(!updatedActions.empty()){
                                    anticipatedPlan.push_back(make_pair(time,updatedActions));
                                    updatedActions.clear();
                                }
                            }
                        }
                        
                        if(!anticipatedPlan.empty()){
                            for(list<pair<double, list<ActionSegment> > >::iterator rItr = relaxedPlan.begin();rItr != relaxedPlan.end();++rItr){
                                orderingTime[rItr->first] = false;
                            }
                            for(map<double,bool>::iterator tItr = orderingTime.begin();tItr!=orderingTime.end();++tItr){
                                double newTime = tItr->first;
                                list<ActionSegment> updatedActions;
                                list<pair<double, list<ActionSegment> > >::iterator rItr = relaxedPlan.begin();
                                const list<pair<double, list<ActionSegment> > >::iterator rEnd = relaxedPlan.end();
                                for (;rItr!=rEnd;++rItr){
                                    double time = rItr->first;
                                    if (time==newTime){
                                        // check if there is a time in anticipating that is not is before
                                        list<ActionSegment> actions = rItr->second;
                                        list<ActionSegment>::iterator aItr = actions.begin();
                                        ...
                                        for(;aItr!=aEnd;++aItr){
                                            int idAction = aItr->first->getID();
                                            VAL::time_spec ts = aItr->second;
                                            map<pair<int,VAL::time_spec>,double>::iterator antItr = anticipating.find(make_pair(idAction,ts));
                                            if(antItr==anticipating.end()){
                                                updatedActions.push_back(*aItr);
                                            }else{
                                                if(antItr->second>time){
                                                    updatedActions.push_back(*aItr);
                                                }
                                            }
                                        }
                                        if(!updatedActions.empty()){
                                            newRelaxedPlan.push_back(make_pair(newTime,updatedActions));
                                            updatedActions.clear();
                                        }
                                    }
                                }
                                
                                list<pair<double, list<ActionSegment> > >::iterator arItr = anticipatedPlan.begin();
                                const list<pair<double, list<ActionSegment> > >::iterator arEnd = anticipatedPlan.end();
                                for (;arItr!=arEnd;++arItr){
                                    double time = arItr->first;
                                    if (time==newTime){
                                        // check if there is a time in anticipating that is not is before
                                        list<ActionSegment> actions = arItr->second;
                                        list<ActionSegment>::iterator aItr = actions.begin();
                                        list<ActionSegment>::iterator aEnd = actions.end();
                                        for(;aItr!=aEnd;++aItr){
                                            int idAction = aItr->first->getID();
                    ...
                    if (earliestTIL.first != INT_MAX) {
                        ...
                        for (int tilID = theState.nextTIL; tilID <= earliestTIL.first; ++tilID) {
                            ...
                            for (; rpItr != rpEnd; ++rpItr) {
                                double time = rpItr->first;
                                //cout << rp
                                if (time < minActionTime && LookAhead::isActive && ExternalSolver::isActive && firstTIL && false){
                                    minActionTime = time;
                                }
                                if (time > tilTime) {
                                    break;
                            ...
                            // check if it's the correct set of helpful action
                            list<ActionSegment> toMantainFirst;
                            bool toDelete = false;
                            list<pair<double, list<ActionSegment> > >::iterator rp2Itr = relaxedPlan.begin();
                            const list<pair<double, list<ActionSegment> > >::iterator rp2End = relaxedPlan.end();
                            for (; rp2Itr != rp2End; ++rp2Itr) {
                                double time = rp2Itr->first;
                                //cout << time << " " <<  minActionTime << endl;
                                if (time <= minActionTime && LookAhead::isActive && ExternalSolver::isActive && firstTIL && false){
                                    toDelete = true;
                                    list<ActionSegment>::iterator lItr = rp2Itr->second.begin();
                                    for(;lItr!=rp2Itr->second.end();++lItr){
                                        //			if (lItr->second==VAL::E_AT_END){
                                        //			     continue;
                                        //			}
                                        list<ActionSegment>::iterator hItr = helpfulActions.begin();
                                        list<ActionSegment>::iterator hEnd = helpfulActions.end();
                                        for(;hItr!=hEnd;++hItr){
                                            if(hItr->first==lItr->first && lItr->second == hItr->second){
                                                toMantainFirst.push_back(*hItr);
                                                //cout << *hItr << endl;
                                            }
                                        }
                                    }
                            ...
                            if (rpItr != rpEnd) {
                                if (EpsilonResolutionTimestamp(rpItr->first,true) == EpsilonResolutionTimestamp(tilTime,true) ) {
                                    ...
                                    /////
                                    if (LookAhead::isActive && !anticipating.empty()){
                                        list<pair<double, list<ActionSegment> > >::iterator rItr = relaxedPlan.begin();
                                        const list<pair<double, list<ActionSegment> > >::iterator rEnd = relaxedPlan.end();
                                        list<ActionSegment> toMantain;
                                        bool toDelete = false;
                                        for(;rItr!=rEnd;++rItr){
                                            // it really before til
                                            double time = rItr->first;
                                            if(time<tilTime && tilID == theState.nextTIL || !anticipating.empty()){// && tilTime <= minActionTime){
                                                
                                                //check if there is some action to anticipate
                                                bool canSkip=false;
                                                
                                                for(list<ActionSegment>::iterator lItr = rItr->second.begin(); lItr!= rItr->second.end();++lItr){
                                                    if(lItr->first){
                                                        int idAction = lItr->first->getID();
                                                        VAL::time_spec ts = lItr->second;
                                                        if(anticipating.find(make_pair(idAction,ts))==anticipating.end()){
                                                            canSkip=true;
                                                        }
                                                    }
                                                }
                                                ...
                                                // check if actions are in helpful actions
                                                list<ActionSegment>::iterator lItr = rItr->second.begin();
                                                list<ActionSegment>::iterator lEnd = rItr->second.end();
                                                for(;lItr!=lEnd;++lItr){
                                                    // check if action is in anticipating
                                                    double updatedTime = time;
                                                    if(lItr->first ){
                                                        int idAction = lItr->first->getID();
                                                        VAL::time_spec ts = lItr->second;
                                                        if(anticipating.find(make_pair(idAction,ts))!=anticipating.end() /* && lItr->second != VAL::E_AT_END*/){
                                                            updatedTime = anticipating[make_pair(idAction,ts)]-0.001;
                                                            if(updatedTime<minActionTime){
                                                                minActionTime=updatedTime;
                                                            }
                                                        }
                                                        
                                                        
                                                    }
                                                    if(updatedTime<tilTime && tilID == theState.nextTIL){
                                                        // check if included in helpful actions
                                                        list<ActionSegment>::iterator hItr = helpfulActions.begin();
                                                        list<ActionSegment>::iterator hEnd = helpfulActions.end();
                                                        // cout << *lItr << endl;
                                                        
                                                        //					cout << *lItr << " " << time << " " << updatedTime << " " << tilTime<< endl;
                                                        for(;hItr!=hEnd;++hItr){
                                                            //cout << "\t*" << *hItr << " " << *lItr << endl;
                                                            if(hItr->first==lItr->first && lItr->second == hItr->second){
                                                                //cout <<*hItr << " is really an helpful action " << endl;
                                                                toMantain.push_back(*hItr);
                                                            }
                                                        }
                                                    }else {
                                                        list<ActionSegment>::iterator hItr = helpfulActions.begin();
                                                        ...
                                                        for(;hItr!=hEnd;++hItr){
                                                            //					    cout << "\t*" << *hItr << " " << *lItr << endl;
                                                            if(hItr->first==lItr->first && lItr->second == hItr->second){
                                                                //						cout <<*hItr << " is not really an helpful action " << endl;
                                                                toDelete = false;
                                                            }
                                                        }
                                        ...
                                        if(toDelete){
                                            helpfulActions.clear();
                                            helpfulActions=toMantain;
                                        }
                                    }
                                    //		    cout << " min Act Time " << minActionTime << " " << tilTime << endl;
                                    if (tilID == theState.nextTIL && tilTime <= minActionTime) {
                                        //cout << " min Act Time " << minActionTime << " " << tilTime << endl;
                                        
                                        if (tilTime < minActionTime && LookAhead::isActive && ExternalSolver::isActive && false){
                                            //			    cout << " inside clearing" << endl;
                                            helpfulActions.clear();
                                        }
                                        helpfulActions.push_back(rpItr->second.back());
                                    }
                    ...
                    if (evaluateDebug || Globals::globalVerbosity & 1048576) {
                        cout << "Relaxed plan for state "<<  statesEvaluated << ", h: "<< h << " :\n";
                        list<pair<double, list<ActionSegment> > >::iterator rpItr = relaxedPlan.begin();
                        const list<pair<double, list<ActionSegment> > >::iterator rpEnd = relaxedPlan.end();
                ...
                bool RPGHeuristic::Private::applyPropositionalEffects(Private::BuildingPayload * const payload, const int & currAct, const VAL::time_spec & currTS, const bool & openEnd, const EpsilonResolutionTimestamp & nlTime)//, MaxDependentInfo & POtime)
                {
                    ...
                    list<Literal*> & addEffects = (currTS == VAL::E_AT ? RPGBuilder::getTILVec()[currAct]->addEffects
                                                   : (currTS == VAL::E_AT_START ? RPGBuilder::getStartPropositionAdds()[currAct] : RPGBuilder::getEndPropositionAdds()[currAct])
                                                   );
                ...
                void RPGHeuristic::Private::recalculateRPGEffects(MinimalState theState, int currAct, const VAL::time_spec  currTS, list<int> &newEffects){
                    MinimalState newState = theState;
                    list<string> parameters = ExternalSolver::externalSolver->getParameters();
                    {
                        /*const*/ list<int>  numericEffects = (currTS == VAL::E_AT
                                                               ? RPGBuilder::getTILVec()[currAct]->rpgNumericEffects
                                                               : (currTS == VAL::E_AT_START ? (*actionsToRPGNumericStartEffects)[currAct]
                                                                  : (*actionsToRPGNumericEndEffects)[currAct]  )  );
                        
                        list<int>::iterator effItr = numericEffects.begin();
                        list<int>::iterator effEnd = numericEffects.end();
                        for(;effItr!=effEnd;++effItr){
                            RPGBuilder::RPGNumericEffect & currEff = RPGBuilder::getNumericEff()[*effItr];
                            int fluentIndex = currEff.fluentIndex;
                            ostringstream fluentNameOs;
                            fluentNameOs << RPGBuilder::getPNE(fluentIndex)->getHead()->getName();
                            string fluentName = fluentNameOs.str();
                            if (std::find(parameters.begin(),parameters.end(),fluentName)!=parameters.end()){
                                continue;
                            }
                            currEff.applyEffectMinMax(theState,newState);
                            //cout << currEff << " recalculated is " << newState.secondMin[currEff.fluentIndex]<< endl;
                        }
                    }
                    
                    MinimalState newestState = newState;
                    //    for (int v = 0; v<RPGBuilder::getPNECount() ; v++){
                    //	cout << *(RPGBuilder::getPNE(v)) << " " << theState.secondMin[v] << endl;
                    //    }
                    list<int>  numericEffects = (currTS == VAL::E_AT
                                                 ? RPGBuilder::getTILVec()[currAct]->rpgNumericEffects
                                                 : (currTS == VAL::E_AT_START ? (*actionsToRPGNumericStartEffects)[currAct]
                                                    : (*actionsToRPGNumericEndEffects)[currAct]  )  );
                    ...
                    list<int>::iterator effEnd = numericEffects.end();
                    ExternalSolver::count=0;
                    // cout << *(RPGBuilder::getInstantiatedOp(currAct)) << endl;
                    for (;effItr!=effEnd;++effItr){
                        RPGBuilder::RPGNumericEffect & currEff = RPGBuilder::getNumericEff()[*effItr];
                        int fluentIndex = currEff.fluentIndex;
                        ostringstream fluentNameOs;
                        ...
                        currEff.applyEffectMinMaxWithExternalEvaluation(newState,newestState);
                        int effID = RPGBuilder::getNumericEff().size();
                        vector<double> wVector(0);
                        vector<int> vVector(0);
                        double constTerm = newestState.secondMin[fluentIndex]-theState.secondMin[fluentIndex];
                        //this is the new effect
                        //cout <<  "\t check here " << *effItr << " " << RPGBuilder::getNumericEff()[*effItr] <<  newestState.secondMin[fluentIndex] << " " << theState.secondMin[fluentIndex] << " " << constTerm << endl;
                        RPGBuilder::RPGNumericEffect newEffect(*effItr,fluentIndex,false,wVector,vVector,0,constTerm);
                        RPGBuilder::changeNumericEff(newEffect,*effItr);
                        //cout <<  "\t" << RPGBuilder::getNumericEff()[*effItr] << endl;
                    }
                    //    for (int v = 0; v<RPGBuilder::getPNECount() ; v++){
                    //	cout << "--" << *(RPGBuilder::getPNE(v)) << " " << newestState.secondMin[v] << endl;
                    //    }
                                                                ...
                                                                const int & limitTo)
                {
                    ...
                    double minDur = EPSILON;
                    double maxDur = EPSILON;
                    
                    if (currTS != VAL::E_AT) {
                        minDur = RPGBuilder::getOpMinDuration(currAct,0);
                        maxDur = RPGBuilder::getOpMaxDuration(currAct,0);
                    }
                    ...
                    
                    {
                        ...
                        list <int> recalculatedEffects;
                        
                        if(ExternalSolver::heuristicActive && ExternalSolver::timeToCalculate/* && RPGHeuristic::statesEvaluated%50 == 0*/){
                            
                            //cout << "recalculating " << RPGHeuristic::statesEvaluated<<endl;
                            
                            list<int>  numericEffectsExternalSolver;
                            list<int>  numericEffectsStandard;
                            
                            if(currTS==VAL::E_AT_START){
                                numericEffectsExternalSolver = (RPGBuilder::getStartExSoEffNumerics())[currAct];
                                numericEffectsStandard = (RPGBuilder::getStartNonExSoEffNumerics())[currAct];
                            }
                            if(currTS==VAL::E_AT_END){
                                numericEffectsExternalSolver = (RPGBuilder::getEndExSoEffNumerics())[currAct];
                                numericEffectsStandard = (RPGBuilder::getEndNonExSoEffNumerics())[currAct];
                            }
                            if(currTS!=VAL::E_AT){
                                if(!numericEffectsExternalSolver.empty()){
                                    recalculateRPGEffects(payload->startState, currAct, currTS, recalculatedEffects);
                                }
                            }
                        }
                        ...
                        /*const*/ list<int> & numericEffects = (currTS == VAL::E_AT
                                                                ...
                                                                : (currTS == VAL::E_AT_START ? (*actionsToRPGNumericStartEffects)[currAct]
                                                                   : (*actionsToRPGNumericEndEffects)[currAct]  )  );
                        if(ExternalSolver::heuristicActive){
                            //numericEffects.merge(recalculatedEffects);                                                                                                                                         
                            list<int>::iterator reIt = recalculatedEffects.begin();
                            list<int>::iterator reEnd = recalculatedEffects.end();
                            for(;reIt!=reEnd;++reIt){
                                //cout << *reIt << " " << RPGBuilder::getNumericEff()[*reIt] << endl;                                                                                                              
                                numericEffects.push_back(*reIt);
                            }
                        }
                        
                        list<int>::/*const_*/iterator numEffItr = numericEffects.begin();
                        const list<int>::/*const_*/iterator numEffEnd = numericEffects.end();
                    ...
                    for (; depItr != depEnd; ++depItr) {
                        ...
                        if (startAct) {
                            ...
                            if ((!epc && !numericEndPreconditionCounts[currAct]) && forbiddenEnd.find(currAct) == forbiddenEnd.end() && nlTime < latestEndAllowed[currAct]) {
                                ...
                                if (updateDebug) {
                                    cout << "\tEnd of action " << currAct << " now has " << endPreconditionCounts[currAct] << " unsatisfied propositional preconditions";
                                    cout << "," << numericEndPreconditionCounts[currAct] << " end numeric preconditions";
                                    if (forbiddenEnd.find(currAct) != forbiddenEnd.end()) {
                                        cout << ", but is forbidden";
                                    }
                                    if (nlTime >= latestEndAllowed[currAct]) {
                                        cout << ", now is later than " << latestEndAllowed[currAct] << "\n";
                                    }
                                }
                            }
                        }
