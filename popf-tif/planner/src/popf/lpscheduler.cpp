...
void LPScheduler::collateRelevantVariablesAndInvariants(InterestingMap & currInterest, CountedConstraintSet & activeInvariants,
        const int & stepID, const VAL::time_spec & currTS, const int & actID, const int & divisionID,
        ...
        vector<map<int, ConstraintSet > > & invariantsThisStepStartsOnVariableI)
{
    ...
    if (currTS == VAL::E_AT) {
        ...
        map<int,double>::const_iterator teItr = tilVec[divisionID]->numEffects.begin();
        const map<int,double>::const_iterator teEnd = tilVec[divisionID]->numEffects.end();
        
        for (; teItr != teEnd; ++teItr) {
            currInterest.insertEffect(teItr->first);
        }
    }
        ...
        for (int s = 0; s < sLim; ++s) {
            ...
            if (varIdx >= 0) {
                ...
                if (varIdx == -3) {                
                    ...
                    entries.push_back(make_pair(endStep, ceItr->weights[s]));                
                    entries.push_back(make_pair(startStep, currW));
                }
                ...
                entries.push_back(make_pair(parent->timestampVars[stepID], currW));                
                isStable = false;
                parent->finalNumericVars[currVar].everHadADurationDependentEffect = true;
            } else if (varIdx == -20) {
                // -total-time, i.e. a ticker
...
public:
        ...
        if (LPScheduler::lpDebug & 1024) {
            ...
            for (int s = 0 ; s < cSize; ++s) {
                ...
                if (csItr->variables[s] == -3) {
                    ...
                    cout << parent->lp->getColName((*applyTo)[csItr->variables[s]]);
                }
        ...
        for (int s = 0 ; s < cSize; ++s) {
            if (csItr->variables[s] < 0) {
                if (csItr->variables[s] == -3) {
                    ...
                    entries.push_back(make_pair(endStep, csItr->weights[s]));
                    entries.push_back(make_pair(startStep, -csItr->weights[s]));
                    
                } else if (csItr->variables[s] == -4) {
                    entries.push_back(make_pair(parent->timestampVars[stepID], csItr->weights[s]));
                } else {
                    std::cerr << "Fatal internal error: variable index " << csItr->variables[s] << " used\n";
                    exit(1);
                }
            } else {
                entries.push_back(make_pair((*applyTo)[csItr->variables[s]], csItr->weights[s]));
                if (parent->assertions) assert(entries.back().second != 0.0);
            }
        }
    ...
    void assignValueToVariable(const pair<int,double> & assignmentEffect) {

        static const vector<pair<int,double> > emptyEntries;
        
        static const int varCount = RPGBuilder::getPNECount();

        const int currVar = assignmentEffect.first;
        ...
        parent->lp->addCol(emptyEntries, assignmentEffect.second, assignmentEffect.second, MILPSolver::C_REAL);
        ...
        if (lpDebug & 1) {
            cout << "TIL assigned " << *(RPGBuilder::getPNE(currVar)) << " the value " << assignmentEffect.second << "\n";            
        }
        ...
        if (!isStable && wasStable) {
            if (lpDebug & 1) {
                cout << "Effect makes variable unstable next time\n";
            }
...
struct LPScheduler::DurationAdder {
    ...
    void operator()(RPGBuilder::DurationExpr* const currDE) {
        ...
        case VAL::E_LESSEQ: {            
            ...
            parent->lp->addRow(entries, -LPinfinity, currDE->constant);
...
//                TILupbo = TILtimestamps[nextTIL];
                ...
                if (currTS == VAL::E_AT_END && !currEvent.getEffects && !tilVec.empty()) {
                ...
                if (currEvent.getEffects && !tilVec.empty() && theState.nextTIL < tilVec.size()) {
                    ...
                    
                    {
                        ...
                        const list<Literal*> & instantPres = (currTS == VAL::E_AT_START ? RPGBuilder::getStartPropositionalPreconditions()[actID] : RPGBuilder::getEndPropositionalPreconditions()[actID]);
                        ...
                        for (; pItr != pEnd; ++pItr) {
                            ...
                            bool allTILs = true;
                            const list<pair<int, VAL::time_spec> > & deletors = RPGBuilder::getNegativeEffectsToActions()[(*pItr)->getStateID()];
...
//                    }
                    ...
                    if (overallWorkingUpper < DBL_MAX) {
                        ...
                        lp->setColUpper(currVar, overallWorkingUpper);
                    }
                }
        ...
        if (currEvent.getEffects) {
            ...
            collateRelevantVariablesAndInvariants(currInterest, activeInvariants, stepID, currTS, actID, currEvent.divisionID,
            ...
            
            {
                ...
                if (currTS == VAL::E_AT) {
                    const map<int,double> & currEffs = RPGBuilder::getTILVec()[currEvent.divisionID]->numEffects;
                    
                    map<int,double>::const_iterator tilEffItr = currEffs.begin();
                    const map<int,double>::const_iterator tilEffEnd = currEffs.end();
                    
                    for (; tilEffItr != tilEffEnd; ++tilEffItr) {
                        adder.assignValueToVariable(*tilEffItr);
                    }
                } else {
                    ...
                    if (workOutFactLayerZeroBoundsStraightAfterRecentAction) {
                        ...
                        list<Literal*> & currPropEffs = (currTS == VAL::E_AT_START ? RPGBuilder::getStartPropositionAdds()[actID]  : RPGBuilder::getEndPropositionAdds()[actID]);
                ...
                for (; geItr != geEnd; ++geItr) {
                    ...
                    if (currTracker.statusOfThisFluent == FluentTracking::FS_NORMAL) {
                        if (!--(currTracker.activeGradientCount)) {
                            ...
                            if ( false && !finalNumericVars[geItr->first].everHadADurationDependentEffect) {
                        ...
                        if (LPScheduler::lpDebug & 1024) {
                            ...
                            for (int s = 0 ; s < cSize; ++s) {
                                ...
                                if ((*invItr)->variables[s] == -3) {
                                    ...
                                    if (finalNumericVars[(*invItr)->variables[s]].lastEffectValueVariable >= 0) {
                                        cout << lp->getColName(finalNumericVars[(*invItr)->variables[s]].lastEffectValueVariable);
                                    } else {
                                        cout << *(RPGBuilder::getPNE((*invItr)->variables[s])) << "<init>";
                                    }
                                }
...
void LPScheduler::initialise()
{
    ...
    /*{
        ...
        for (; sfItr != sfEnd; ++sfItr) {
            ...
            for (; vItr != vEnd; ++vItr) {
              list<pair<int, VAL::time_spec> >::const_iterator pItr = RPGBuilder::getEffectsToActions(sfItr->first).begin();
              const list<pair<int, VAL::time_spec> >::const_iterator pEnd = RPGBuilder::getEffectsToActions(sfItr->first).end();
              
              for (; pItr != pEnd; ++pItr) {
                  if (pItr->second == VAL::E_AT_START) {
                      interesting[pItr->first][0].insertEffect(*vItr);
                  } else if (pItr->second == VAL::E_AT_END) {
...
bool LPScheduler::isSolution(const MinimalState & state, list<FFEvent> & header, list<FFEvent> & now)
{
    ...
    if (TemporalAnalysis::futureTILsMightInterfereWithGoals(state.nextTIL)) {
        if (lpDebug & 1) {
            cout << "Not a goal state: future TILs might interfere with the goals\n";
        }
        return false;
    }            
...
//    lp->resize(rowCount + goalCount, colCount);
    ...
    for (; gItr != gEnd; ++gItr) {
        ...
        int v;
        for (int s = 0 ; s < cSize; ++s) {
            if ((*gItr)->variables[s] == -5) {
                if (RPGBuilder::getMetric()->minimise) {
                    offset -= ((*gItr)->weights[s] * Globals::bestSolutionQuality);
                } else {
                    offset += ((*gItr)->weights[s] * Globals::bestSolutionQuality);
                }
                continue;
            } else if ((*gItr)->variables[s] == -21) {
                if (RPGBuilder::getMetric()->minimise) {
                    offset += ((*gItr)->weights[s] * Globals::bestSolutionQuality);
                } else {
                    offset -= ((*gItr)->weights[s] * Globals::bestSolutionQuality);
    ...
    for (int act = 0; act < actCount; ++act) {
        ...
        for (int cep = 0; ceItr != ceEnd; ++ceItr, ++cep) {
            ...
            for (; effItr != effEnd; ++effItr) {
                ...
                assert(effItr->second == VAL::E_AT_START || actEndAt != -1);
...
bool LPScheduler::checkTimewindow(ChildData* cd)
{
    bool result = (cd && cd->propagateNewEdges());
    delete cd;
    return result;
...
ParentData * LPScheduler::prime(list<FFEvent> & header, const TemporalConstraints * const cons, list<StartEvent> & open, const bool includeMetric)
{
    ...
    for (; hItr != hEnd; ++hItr, ++i) {
        ...
        if (hItr->action) {
            ...
            toReturn->setTIL(i, hItr->divisionID);
        }
                                       ...
                                       const TemporalConstraints * const cons, const int & stepID)
{
    ...
    if (startGap > stepID) { // if we're only triggering an existing action within the plan
        ...
        if (TILupbo == DBL_MAX && nextTIL < RPGBuilder::getTILVec().size()) {            
            const ActionSegment tempSeg(s.action, s.time_spec, s.divisionID, set<int>());
            TILupbo = TemporalAnalysis::upperBoundOnActionIfNextTILIs(tempSeg, nextTIL);            
        }
        ...
        if (TILupbo < endMax) {
            if (TILupbo < endMin) {
                if (spawnDebug) cout << "Due to future TILs (the next with index " << nextTIL << "), the just-triggered step has to come before time " << TILupbo << ", but it cannot come earlier than " << endMin << endl;
                delete toReturn;
                return 0;
            }            
            if (spawnDebug) cout << "Due to future TILs (the next with index " << nextTIL << "), adding edge from time zero to the just-triggered end at " << endLocation << " to set its max ts to " << TILupbo << endl;
            toReturn->addNewEdge(BFEdge(-1, endLocation, endMin, TILupbo));            
        }
    ...
    if (s.time_spec == VAL::E_AT_START) {
        ...
        if (startMax > TILupbo) {
            if (spawnDebug) cout << "Due to future TILs (the next with index " << nextTIL << "), upper-bound on the start becomes " << TILupbo << endl;
            startMax = TILupbo;
        }
    ...
    if (floydDoubleCheck) {
        ...
        for (int m = 1; m < mSize; ++m) {
            ...
            if (eItr != temporaryEdges.end()) {
                ...
                for (; pItr != pEnd; ++pItr) {
                    if ((pItr->second ? -0.001 : 0) < matrix[m][pItr->first + 1]) {
                        matrix[m][pItr->first + 1] = (pItr->second ? -0.001 : 0);
                        if (noisy) cout << "Edge from " << m - 1 << " to " << pItr->first << " - " << (pItr->second ? -0.001 : 0) << " due to recorded predecessors" << endl;
                    }
                }
