...
void RPGBuilder::findTimewindows(){

    const int opCount = instantiatedOps.size();
    timewindows = vector<bool>(opCount, false);
    propositionTimewindows = vector<LiteralSet>(opCount);
    for (int i = 0; i < opCount; ++i) {
        ...
        copyLiteralListToSet(actionsToEndEffects[i], endAdds);
        LiteralSet candidates;        
        for (LiteralSet::iterator it = startAdds.begin(); it!=startAdds.end();++it){
            Literal* l = *it;
            for(LiteralSet::iterator eIt = endDeletes.begin(); eIt!=endDeletes.end();++eIt){
                Literal* e = *eIt;
                if(l==e){
                    candidates.insert(l);
                    break;
        ...
        propositionTimewindows[i]=candidates;
        if(candidates.size()){
            timewindows[i] = true;
...
void RPGBuilder::findOneshotTimewindows(MinimalState & refState){
    findTimewindows();
    bool localDebug = false;
    const int opCount = instantiatedOps.size();
    oneshotActions = vector<bool> (opCount,false);
    oneshotTimewindowActions = vector<bool> (opCount, false);
    actionsDependentFromOneshotTimewindows = vector<set<int> >(opCount);
    for (int i = 0; i< opCount; ++i){
	    if (rogueActions[i]) continue;
	    oneshotActions[i] = RPGBuilder::howManyTimesOptimistic(i,refState)==1;
	    oneshotTimewindowActions[i] = oneshotActions[i] && timewindows[i];
	    if(localDebug && oneshotTimewindowActions[i]){
		cout << "one shot time window " << *(RPGBuilder::getInstantiatedOp(i)) << endl;
	    }
	}
	     findDependencyTimewindowsActions();

};

    void RPGBuilder::findDependencyTimewindowsActions(){
	const int opCount = instantiatedOps.size();
	for(int i = 0; i<opCount; ++i){
	    list<Literal*> invariantPreconditions = getInvariantPropositionalPreconditions()[i];
	    for(int j = 0; j<opCount;++j){
		if(oneshotTimewindowActions[j]){
		    LiteralSet timewindowPreconditions = propositionTimewindows[j];
		    LiteralSet::iterator nIt = timewindowPreconditions.begin();
		    LiteralSet::iterator nEnd = timewindowPreconditions.end();
		    LiteralSet satisfied;
		    if(find(invariantPreconditions.begin(),invariantPreconditions.end(),*nIt)!=invariantPreconditions.end()){
			actionsDependentFromOneshotTimewindows[i].insert(j);
			//cout << *(RPGBuilder::getInstantiatedOp(i)) << " depends on " << *(RPGBuilder::getInstantiatedOp(j)) << endl;
		    }
		}
	    }
	}
...
//              cout << "Revisiting " << *(getInstantiatedOp(*riItr)) << " due to one-shot literal\n";
    ...
    findOneshotTimewindows(refState);
...
private:
    ...
    vector<set<int> > fluentAffectedByTIL;
        ...
        for (int f = 0; f < factCount; ++f) {
            if (!presToActions[f].empty()) {
                ...
                for (; acItr != acEnd; ++acItr) {
                    if (acItr->second == VAL::E_AT_START) {
...
public:
    ...
    CommonRegressionData(const int & lc, const int  & fc, const int & ac, const int & dc)
        : fluentCount(fc), literalSeenForGoalAtTime(lc), fluentSeenForGoalAtTime(fc), opAppliedForGoalAtTime(ac), dpAppliedForGoalAtTime(dc),
          fluentAffectedBy(fc), fluentAffectedByTIL(fc),
            ...
            for (int i = 0; numEffItr != numEffEnd; ++numEffItr, ++i) {
                ...
                if (proceed) {
                    ...
                    for (; accItr != accEnd; ++accItr) {
                        if (accItr->second == VAL::E_AT) {
                            fluentAffectedByTIL[affFluent].insert(accItr->first);
                        } else {
    ...
    map<int, set<VAL::time_spec> > & actionsRelevantToFluent(const int & currVar) {
        return fluentAffectedBy[currVar];
    }

    set<int> & TILSrelevantToFluent(const int & currVar) {
        return fluentAffectedByTIL[currVar];
    }
...
void RPGBuilder::pruneIrrelevant(const int & operatorID)
{
    ...
    if (!actionsToRPGNumericInvariants.empty()) {
    ...
    if (!actionsToRPGNumericEndPreconditions.empty()) {
    ...
    while (!data.empty()) {
        ...
        if (currPair.fact != -1) {
            ...
            */
            ...
            map<int, set<VAL::time_spec> >::iterator faItr = data.actionsRelevantToFluent(currVar).begin();
            const map<int, set<VAL::time_spec> >::iterator faEnd = data.actionsRelevantToFluent(currVar).end();
...
void RPGBuilder::separateOptimisationTILs()
{
    ...
    for (int ti = 0; tilItr != tilEnd; ++ti) {
        ...
        FakeTILAction metricParts(tilItr->duration, LiteralSet(), LiteralSet(), map<int,double>());
        
        bool metricOnly = tilItr->numEffects.empty();
        bool anyMetric = false;
        {
            ...
            while (elItr != elEnd) {
                if (   !preconditionsToActions[(*elItr)->getStateID()].empty()
                    ...
                    || literalGoalSet.find(*elItr) != literalGoalSet.end()             ) {
                    metricOnly = false;
                    ++elItr;
                } else {
        ...
        {
            const map<int,double> & effList = tilItr->numEffects;
            
            map<int,double>::const_iterator effItr = effList.begin();
            const map<int,double>::const_iterator effEnd = effList.end();
            
            for (; effItr != effEnd; ++effItr) {
                if (NumericAnalysis::getDominanceConstraints()[effItr->first] == NumericAnalysis::E_IRRELEVANT) {
                    continue;
                }
                metricOnly = false;
            }
...
void RPGBuilder::findSemaphoreFacts()
{
    ...
    for (int litID = 0; litID < factCount; ++litID) {
        ...
        {
            const list<pair<int, VAL::time_spec> > & dependents = preconditionsToActions[litID];
            ...
            for (; depItr != depEnd; ++depItr) {
                ...
                if (depItr->second == VAL::E_OVER_ALL) {
                    if (localDebug) {
                ...
                {
                    const list<Literal*> & simultaneousDels = (depItr->second == VAL::E_AT_START ? actionsToStartNegativeEffects[depItr->first] : actionsToEndNegativeEffects[depItr->first]);
                ...
                for (int pass = 0; pass < 2; ++pass) {
                    if (pass == 0 && depItr->second == VAL::E_AT_END) {
                        continue;
                    }
        ...
        
        {
            ...
            const list<pair<int, VAL::time_spec> > & affectors = negativeEffectsToActions[litID];
            
            list<pair<int, VAL::time_spec> >::const_iterator affItr = affectors.begin();
            const list<pair<int, VAL::time_spec> >::const_iterator affEnd = affectors.end();
            
            for (; affItr != affEnd; ++affItr) {
                const list<Literal*> & simultaneousPres = (affItr->second == VAL::E_AT_START ? actionsToStartPreconditions[affItr->first] : actionsToEndPreconditions[affItr->first]);
                ...
                for (int pass = 0; pass < 2; ++pass) {
                    if (pass == 0 && affItr->second == VAL::E_AT_END) {
                        continue;
                    }
        ...
        
        {
            ...
            const list<pair<int, VAL::time_spec> > & affectors = effectsToActions[litID];
            ...
            for (; affItr != affEnd; ++affItr) {
                ...
                for (int pass = 0; pass < 2; ++pass) {
                    if (pass == 0 && affItr->second == VAL::E_AT_START) {
                        continue;
                    }
    ...
    
    {
        ...
        for (int rnp = 0; rnp < rnpCount; ++rnp) {
            ...
            const list<pair<int, VAL::time_spec> > & deps = rpgNumericPreconditionsToActions[rnp];
            
            list<pair<int, VAL::time_spec> >::const_iterator depItr = deps.begin();
            const list<pair<int, VAL::time_spec> >::const_iterator depEnd = deps.end();
    ...
    
    {
        ...
        for (int rne = 0; rne < rneCount; ++rne) {
            ...
            const list<pair<int, VAL::time_spec> > & deps = rpgNumericEffectsToActions[rne];
            ...
            for (; depItr != depEnd; ++depItr) {
                ...
                } else if (depItr->second == VAL::E_AT) {
                    if (localDebug) {
                        cout << "Attached to TIL at " << RPGBuilder::getTILVec()[depItr->first]->duration << endl;
                    }
                } else {                    
                    std::cerr << "Internal error: have found an instantaneous effect that doesn't apply to a start or end action, or TIL\n";
                    exit(1);
                }
