...
:duration (= ?duration 1)
:condition (and (over all (at ?x ?p)) (over all (at ?z ?p)) (at start (available ?x)) (at start (at ?y ?p)) (at start (on ?y ?z)) (at start (clear ?y)))
...
:parameters (?x - hoist ?y - crate ?z - surface ?p - place)
:duration (= ?duration 1)
:condition (and (over all (at ?x ?p)) (over all (at ?z ?p)) (at start (clear ?z)) (at start (lifting ?x ?y)))
...
:duration (= ?duration 3)
:condition (and (over all (at ?x ?p)) (over all (at ?z ?p)) (at start (lifting ?x ?y)))
