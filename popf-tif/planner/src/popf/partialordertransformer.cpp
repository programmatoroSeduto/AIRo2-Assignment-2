...
namespace Planner
{
    ...
    // tif interaction
    void addOrderingEndActionTIF(MinimalState & theState, int localFluentIndex, int stepID){
        bool localDebug = false;
        FluentInteraction & currFI = theState.temporalConstraints->lastStepToTouchPNE[localFluentIndex];
        map<int,map<int,int> > & relatedVariables = currFI.fluentsRelated;
        
        map<int,map<int,int> >::iterator rvIt = relatedVariables.begin();
        map<int,map<int,int> >::iterator rvEnd = relatedVariables.end();

        for(;rvIt!=rvEnd;++rvIt){
            int index = rvIt->first;
            map<int,int> endStart = rvIt->second;
            const set<int> & futureSet = theState.temporalConstraints->lastStepToTouchPNE[index].futureEndInteractions;
            for ( set<int>::iterator fsIt = futureSet.begin(); fsIt!=futureSet.end(); ++fsIt){
                if (localDebug){
                    cout << "\tRequesting ordering "<< *fsIt << " to come after " << stepID << endl;
                    cout << "\tBecause " <<  (*RPGBuilder::getPNE(localFluentIndex)) << " and " << (*RPGBuilder::getPNE(index)) << "interact in the future at step " << *fsIt << endl;
                }
                theState.temporalConstraints->addOrdering(stepID,*fsIt,true);
    ...
    // tif interaction
    void addOrderingFluentRelated(MinimalState & theState, int localFleuntIndex, int stepID){
        
        bool localDebug = false;
        map<int,map<int,int> > & relatedVariables = theState.temporalConstraints->lastStepToTouchPNE[localFleuntIndex].fluentsRelated;
        ...
        for(;rvIt!=rvEnd;++rvIt){
            int index = rvIt->first;
            map<int,int> endStart = rvIt->second;
            int lastInsEff = theState.temporalConstraints->lastStepToTouchPNE[index].lastInstantaneousEffect;
            if (!endStart.empty() && stepID != lastInsEff && lastInsEff!=-1){
                if (localDebug){
                    cout << "\tRequesting ordering "<< stepID << " to come after " << lastInsEff << endl;
                    cout << "\tBecause interaction "<< (*RPGBuilder::getPNE(localFleuntIndex)) << " and " << (*RPGBuilder::getPNE(index)) << endl;
                }
                theState.temporalConstraints->addOrdering(lastInsEff,stepID,true);
    ...
    bool POTHelper_updateForInputsToInstantaneousNumericEffectsExSo(set<int> & mentioned)
    {
        bool toReturn = false;
        static const int PNEcount = RPGBuilder::getPNECount();
        list<string> parameters = ExternalSolver::externalSolver->getParameters();
        for (int i=0; i< PNEcount; i++){
            string fluentName = RPGBuilder::getPNE(i)->getHead()->getName();
            if (find(parameters.begin(),parameters.end(),fluentName)!=parameters.end()) {
                mentioned.insert(i);
                toReturn = true;
            }
        }
    ...
    bool POTHelper_updateForInputsToInstantaneousNumericEffectsExSo(set<int> & mentioned, list<int> & reserve)
    {
        ...
        for (; precItr != precEnd; ++precItr) {
            RPGBuilder::RPGNumericEffect & currEff = RPGBuilder::getNumericEff()[*precItr];
            string sName = RPGBuilder::getPNE(currEff.fluentIndex)->getHead()->getName();
            list<string> dependencies = ExternalSolver::externalSolver->getDependencies();
            list<string> parameters = ExternalSolver::externalSolver->getParameters();
            
            if (find(dependencies.begin(),dependencies.end(),sName)!=dependencies.end() || find(parameters.begin(),parameters.end(),sName)!=parameters.end()){
                for (int i=0; i< PNEcount; i++){
                    string fluentName = RPGBuilder::getPNE(i)->getHead()->getName();
                    if (find(dependencies.begin(),dependencies.end(),fluentName)!=dependencies.end() || find(parameters.begin(),parameters.end(),fluentName)!=parameters.end() ) {
                        mentioned.insert(i);
                        toReturn = true;
                    }
                }
                                                                   ...
                                                                   const int & stepID, list<int> & reserve, const double & minDur, const double & maxDur)
    {
        ...
        for (; precItr != precEnd; ++precItr) {
            ...
            if (NumericAnalysis::getDominanceConstraints()[currEff.fluentIndex] == NumericAnalysis::E_IRRELEVANT && !ExternalSolver::isActive) {
                continue;
            }
            ...
            {
                addOrderingFluentRelated(theState,localFluentIndex,stepID);
        ...
        if (act.first) {
            ...
            = (act.second == VAL::E_AT_END ? NumericAnalysis::getTimeDependentEndCosts()[act.first->getID()]
               : NumericAnalysis::getTimeDependentStartCosts()[act.first->getID()]);
            
            
            if (dynamicCosts) {
                //cout << "Got dynamic costs on " << *(act.first) << endl;
                double timeNow = 0.0;
    ...
    void POTHelper_updateForOutputsFromTILNumericEffects(MinimalState & theState, const vector<double> & minTimestamps, const ActionSegment & act,
                                                         const int & stepID, const map<int,double> & assignments) {
        static const int PNEcount = RPGBuilder::getPNECount();
        
        map<int,double>::const_iterator neItr = assignments.begin();
        const map<int,double>::const_iterator neEnd = assignments.end();
        
        for (; neItr != neEnd; ++neItr) {
            
            const int localFluentIndex = neItr->first;
            
            if (NumericAnalysis::getDominanceConstraints()[neItr->first] == NumericAnalysis::E_IRRELEVANT && !ExternalSolver::isActive) {
                continue;
            }
            ...
            
            {
                ...
                for (; fsItr != fsEnd; ++fsItr) {
                    if (applyDebug) {
                        cout << "\tRequesting ordering to come before" << *fsItr << ", a future end with a pre/eff on " << *(RPGBuilder::getPNE(localFluentIndex)) << "\n";
                    }
                    theState.temporalConstraints->addOrdering(stepID, *fsItr, true);
                }
            }
            
            {
                ...
                for (; psItr != psEnd; ++psItr) {
                    if (*psItr != stepID) {
                        if (applyDebug) {
                            cout << "\tRequesting ordering to come after" << *psItr << ", a past step with a pre/duration on " << *(RPGBuilder::getPNE(localFluentIndex)) << "\n";
                        }
                        theState.temporalConstraints->addOrdering(*psItr, stepID, true);
                    }
                }
            ...
            
            {
                ...
                addOrderingFluentRelated(theState,localFluentIndex,stepID);
                if (ExternalSolver::isActive){
                    if (RPGBuilder::getSpecialExSo().find(localFluentIndex)!=RPGBuilder::getSpecialExSo().end() || RPGBuilder::getDependentExSo().find(localFluentIndex)!=RPGBuilder::getDependentExSo().end()){
                        set<int> initialVariables;
                        initialVariables.insert(RPGBuilder::getDependentExSo().begin(),RPGBuilder::getDependentExSo().end());
                        initialVariables.insert(RPGBuilder::getSpecialExSo().begin(),RPGBuilder::getSpecialExSo().end());
                        for (set<int>::iterator it = initialVariables.begin();it !=initialVariables.end();++it ){
                            addOrderingEndActionTIF(theState,*it,stepID);
                            
                        }
                    }
                }
                
                addOrderingEndActionTIF(theState,localFluentIndex,stepID);
                ...
                for (; isItr != isEnd; ++isItr) {
                    if (stepID != isItr->second) {
                        if (applyDebug) {
                            cout << "\tRequesting ordering to come after " << isItr->second << ", the start of an invariant on " << *(RPGBuilder::getPNE(localFluentIndex)) << "\n";
                        }
                        theState.temporalConstraints->addOrdering(isItr->second, stepID, true);
                    }
                    ...
                    if (stepID != isItr->first) {
                        if (applyDebug) {
                            cout << "\tRequesting ordering to come after " << isItr->second << ", the end of an invariant on " << *(RPGBuilder::getPNE(localFluentIndex)) << "\n";
                        }
                        theState.temporalConstraints->addOrdering(stepID, isItr->first, true);
                    }
                }
            ...
            theState.secondMin[localFluentIndex] = neItr->second;
            theState.secondMax[localFluentIndex] = neItr->second;
    ...
    void POTHelper_warnForNumericEffects(MinimalState & theState, const int & stepID, list<int> & reserve, const int & minDur, const int & maxDur)
    {
        ...
        for (; precItr != precEnd; ++precItr) {
            ...
            if (NumericAnalysis::getDataOnWhichVariablesHaveOrderIndependentEffects()[currEff.fluentIndex] != NumericAnalysis::E_ORDER_DEPENDENT
                && NumericAnalysis::getDominanceConstraints()[currEff.fluentIndex] == NumericAnalysis::E_METRICTRACKING && !ExternalSolver::isActive) {
                continue;
            }
    ...
    void registerFluentRelated(MinimalState & theState, RPGBuilder::ArtificialVariable & currAV, int startStepID, int  endStepID, const bool & registerNotDeregister, bool & first){
        bool localDebug = false;
        static const int PNEcount = RPGBuilder::getPNECount();
        for (int i = 0; i< currAV.size; ++i){
            const int lv = currAV.fluents[i];
            int id = findIDVariable(lv);
            int n = 0;
            set<int> additionalSet;
            set<int>::iterator additional;
            if (ExternalSolver::isActive && first){
                //list<string> special = ExternalSolver::externalSolver->getAffected();
                //list<string> dep = ExternalSolver::externalSolver->getDependencies();
                if (RPGBuilder::getSpecialExSo().find(id)!=RPGBuilder::getSpecialExSo().end() || RPGBuilder::getDependentExSo().find(id)!=RPGBuilder::getDependentExSo().end()){
                    if (localDebug)
                        cout <<(*RPGBuilder::getPNE(id)) << " is a special variable" << endl;
                    n += RPGBuilder::getDependentExSo().size() + RPGBuilder::getSpecialExSo().size();
                    additionalSet.insert(RPGBuilder::getDependentExSo().begin(),RPGBuilder::getDependentExSo().end());
                    additionalSet.insert(RPGBuilder::getSpecialExSo().begin(),RPGBuilder::getSpecialExSo().end());
                }
                
                additional = additionalSet.begin();
                first = false;
            }
            for (int j =0; j<currAV.size+n; ++j){

                int idj = -1;
                if (j<currAV.size){
                    idj = (findIDVariable(currAV.fluents[j]));
                }else{
                    idj = (*additional);
                    ++additional;
                }
                ...
                if (idj!=id){
                    map<int, map<int,int> > &fluentsRelated = theState.temporalConstraints->lastStepToTouchPNE[id].fluentsRelated;
                    if(registerNotDeregister){
                        if (localDebug){
                            cout << "\tAdd relation between " << (*RPGBuilder::getPNE(id)) << " and " <<  (*RPGBuilder::getPNE(idj)) << endl;
                        }
                        if (fluentsRelated.find(idj)!=fluentsRelated.end()){
                            fluentsRelated[idj].insert(make_pair(endStepID,startStepID));
                        }else{
                            map<int,int> endstart;
                            endstart.insert(make_pair(endStepID,startStepID));
                            fluentsRelated.insert(make_pair(idj,endstart));
                        }
                    }else{
                        if (localDebug){
                            cout << "\tDelete relation between " << (*RPGBuilder::getPNE(id)) << " and " <<  (*RPGBuilder::getPNE(idj)) << endl;
                        }
                        if (fluentsRelated.find(idj)!=fluentsRelated.end()){
                            fluentsRelated[idj].erase(endStepID);
                        }
                    }
                }
            }
        ...
        for (; precItr != precEnd; ++precItr) {
            ...
            for (int pass = 0; pass < 2; ++pass) {
                ...
                if (currVar < PNEcount) {
                    ...
                    RPGBuilder::ArtificialVariable & currAV = RPGBuilder::getArtificialVariable(currVar);
                    registerFluentRelated(theState,currAV,endStepID,startStepID,registerNotDeregister, first);
                    for (int i = 0; i < currAV.size; ++i) {
                        const int lv = currAV.fluents[i];
                        if (lv < PNEcount) {
                            mentioned.insert(lv);
                                                        ...
                                                        const bool & inPlace, const double & minDur, const double & maxDur)
    {
        ...
        if (!a.first) { // applying a TIL
            ...
            for (; workOn->nextTIL <= a.divisionID; ++(workOn->nextTIL)) {
                ...
                const map<int,double> & numEffects = tilVec[workOn->nextTIL]->numEffects;
                
                if (!numEffects.empty()) {
                    set<int> mentioned;
                    
                    map<int,double>::const_iterator neItr = numEffects.begin();
                    const map<int,double>::const_iterator neEnd = numEffects.end();
                    
                    for (; neItr != neEnd; ++neItr) {
                        mentioned.insert(neItr->first);
                    }
                    bool effectOnExternalSolver = false;
                    if(ExternalSolver::isActive){
                        effectOnExternalSolver = POTHelper_updateForInputsToInstantaneousNumericEffectsExSo(mentioned);
                    }
        ...
        if (a.second == VAL::E_AT_START) {
            ...
            if (RPGBuilder::getRPGDEs(actID).empty()) { // non-temporal action
                ...
                bool effectOnExternalSolver = false;
                {
                    ...
                    if(ExternalSolver::isActive){
                        effectOnExternalSolver = POTHelper_updateForInputsToInstantaneousNumericEffectsExSo(mentioned, RPGBuilder::getStartEffNumerics()[actID]);
                    }
                    POTHelper_updateForNumericVariables(*workOn, workOn->planLength, workOn->planLength, true, mentioned);
                }
                if(!ExternalSolver::isActive || !effectOnExternalSolver)
        ...
        
        {
            ...
            bool effectOnExternalSolver = false;
            if(ExternalSolver::isActive){
                effectOnExternalSolver = POTHelper_updateForInputsToInstantaneousNumericEffectsExSo(mentioned,RPGBuilder::getEndEffNumerics()[actID]);
            }
