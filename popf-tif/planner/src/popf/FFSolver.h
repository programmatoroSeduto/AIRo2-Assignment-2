...
public:
    ...
    ExtendedMinimalState()
        : decorated(new MinimalState()), timeStamp(0.0), stepBeforeTIL(-1), tilFanIn(0), hasBeenDominated(false), idParent(-1), id(0) {
    }
    ...
    ExtendedMinimalState(const ExtendedMinimalState & e, MinimalState * const ms)
        : decorated(ms), startEventQueue(e.startEventQueue), timeStamp(e.timeStamp),
          ...
          hasBeenDominated(e.hasBeenDominated), idParent(e.idParent), id(e.id)  {
...
public:
    ...
    static bool restartBFSAfterEachNewBestSolution;
    ...
    static Solution searchBFS(bool &runBestFirstSearch, SearchQueue &searchQueue, set<int>& goals, auto_ptr<StateHash>& visitedStates,set<int>& numericGoals, auto_ptr<StatesToDelete>& statesKept, bool &reachedGoal, HTrio& bestHeuristic, SearchQueue & oldSearchQueue, SearchQueue &backupQueue, bool modifiedSearch = false, double whenToStop = 0, int bestH = 0);
    static void insertStateBackupQueue(auto_ptr<SearchQueueItem>& succ, SearchQueue &backupQueue, bool& WAStart, bool isTIL,auto_ptr<StatesToDelete>& statesKept);
