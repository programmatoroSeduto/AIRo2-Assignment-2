...
void NumericAnalysis::considerConflictingDominanceThroughEffects(vector<dominance_constraint> & workingDominanceConstraints)
{
    ...
    for (int actID = 0; actID < actCount; ++actID) {
        ...
        if ((ddEffs == actionToDurationDependentEffects.end() || ddEffs->first > actID) & !leffs) {
            // action has no duration-dependent effects or continuous effects
            continue;
        }
        
        /*{
            cout << "Looking at " << *(RPGBuilder::getInstantiatedOp(actID)) << endl;
            if (leffs) {
                cout << "- Has continuous effects\n";
            }
            if (ddEffs != actionToDurationDependentEffects.end() && ddEffs->first == actID) {
                cout << "- Has duration-dependent effects\n";
            }
        }*/
        ...
        for (int pass = 0; pass < 2; ++pass) {
            ...
            if (currPre.LHSVariable < 2 * pneCount) {
                ...
                for (int s = 0; s < currAV.size; ++s) {
                    if (currAV.fluents[s] == -5 || currAV.fluents[s] == -21) {
                        safe = false;
                        break;
                    }
                }
...
void NumericAnalysis::findWhichVariablesHaveOrderIndependentEffects()
{
    ...
    
    {
        ...
        for (int til = 0; til < tilCount; ++til) {
            map<int,double>::const_iterator teItr = tilVec[til]->numEffects.begin();
            const map<int,double>::const_iterator teEnd = tilVec[til]->numEffects.end();
            for (; teItr != teEnd; ++teItr) {
                allEffectsAreOrderIndependent[teItr->first] = E_ORDER_DEPENDENT;
            }
        }
    }
...
void NumericAnalysis::findVariablesThatAreTickers()
{
...

    {
        ...
        for (int til = 0; til < tilCount; ++til) {
            ...
            const map<int,double>::const_iterator teEnd = tilVec[til]->numEffects.end();
            for (; teItr != teEnd; ++teItr) {
                tickerVariables.erase(teItr->first);
            }
        }
    }
...
void NumericAnalysis::findVariableBounds()
{
    ...
    vector<pair<double,double > > tilInducedBounds(pneCount);
    
    for (int v = 0; v < pneCount; ++v) {
        tilInducedBounds[v].first = initialValues[v];
        tilInducedBounds[v].second = initialValues[v];
    }
    
    {
        ...
        for (int til = 0; til < tilCount; ++til) {
            ...
            for (; teItr != teEnd; ++teItr) {
                if (teItr->second > tilInducedBounds[teItr->first].second) {
                    tilInducedBounds[teItr->first].second = teItr->second;
                }
                if (teItr->second < tilInducedBounds[teItr->first].first) {
                    tilInducedBounds[teItr->first].first = teItr->second;
                }                                                
            }
        }
    }
    ...
    while (!needToVisit.empty()) {
        ...
        for (; vItr != vEnd; ++vItr) {
            ...
            variableBounds[*vItr] = tilInducedBounds[*vItr]; // initial values modified with TIL assignments
...
vector<bool> NumericAnalysis::variablesOnlyAssignedConstants;

void NumericAnalysis::findVariablesOnlyAssignedConstants()
{
    variablesOnlyAssignedConstants.resize(RPGBuilder::getPNECount(), true);
    
    const vector<RPGBuilder::RPGNumericEffect> & numEffs = RPGBuilder::getNumericEff();
    
    const int effCount = numEffs.size();
    
    for (int eID = 0; eID < effCount; ++eID) {
        if (!numEffs[eID].isAssignment) {
            variablesOnlyAssignedConstants[numEffs[eID].fluentIndex] = false;
        } else if (numEffs[eID].size) {
            variablesOnlyAssignedConstants[numEffs[eID].fluentIndex] = false;
        }
    }
    
    /// TODO: check continuous effects, too
}
