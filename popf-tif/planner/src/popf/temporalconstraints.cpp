...
void TemporalConstraints::extend(const int & extendBy)
{
    ...
    void TemporalConstraints::printTC(){
	for(int i=0;i<stepsComeBeforeThisOne.size();++i){
	    if (stepsComeBeforeThisOne[i]){
		map<int,bool> mapStep = *stepsComeBeforeThisOne[i];
		map<int,bool>::iterator msIt = mapStep.begin();
		map<int,bool>::iterator msEnd = mapStep.end();
		cout << "step " << i << " :" << std::endl << "\t";
		for(;msIt!=msEnd;++msIt){
		    cout << msIt->first << "(" << msIt->second <<") ";
		}
		cout << std::endl;
	    }
	}
